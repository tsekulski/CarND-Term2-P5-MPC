 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (size_t t = 0; t < N; t++) {
      fg[0] += 50*CppAD::pow(vars[cte_start + t], 2); //multiplying by 50 to focus on minimizing cte and epsi
      fg[0] += 50*CppAD::pow(vars[epsi_start + t], 2);//multiplying by 50 to focus on minimizing cte and epsi
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs, vector<double>& mpc_x_vals, vector<double>& mpc_y_vals) {
  ...
  //Store mpc x and y values - to display the MPC predicted trajectory (green line)
  //vector<double> mpc_x_vals; //these are passed by reference
  //vector<double> mpc_y_vals; //these are passed by reference
